...
public class GeneratorWindow implements ActionListener 
{
	...
	protected int do_generatorButton_actionPerformed(ActionEvent e) 
	{
		...
		if(list.isSelectedIndex(0)) //Rectangulo
		{
			...
			{
				errors.setText("Error, valor incorrecto en campo de texto 4");
			    c = null; 
			    return 0;
			}
			try
			{
				height = Double.parseDouble(tf5.getText());
			}catch(Exception p)
			{
			...
			try
			{
				width = Double.parseDouble(tf6.getText());
			}catch(Exception p)
			{
			...
			try
			{
				depth = Double.parseDouble(tf7.getText());
			}catch(Exception p)
			{
			...
			try
			{
				size = Double.parseDouble(tf8.getText());
			}catch(Exception p)
			{
		...
		if(list.isSelectedIndex(2)) //Plano
		{
			try
			{
				x = Double.parseDouble(tf1.getText());
			...
			new Plane(x,y,z,depth,width,c,size);
		}
		if(list.isSelectedIndex(4)) //Terreno
		{
			try
			{
				x = Double.parseDouble(tf1.getText());
			...
			new Terrain(x,y,z,depth,width,c,size,height);
		}
